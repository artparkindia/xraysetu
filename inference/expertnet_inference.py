...
test_set_regular = DataGenerator.DataGenerator(image_list, path=image_path, shuffle=False, test_set=True)
test_set_mobilenet = DataGenerator.DataGenerator(image_list, path=image_path, shuffle=False, test_set=True, input_shape=(224, 224, 3))
...
def get_pred_result():
    all_preds_class = np.empty((len(image_list), len(saved_model_name)))
    all_preds_bbox = np.empty((len(image_list), len(saved_model_name), 4))
...
for img_num, res, bbox in zip(image_list, voice_vote, bbox_nms_pred):    
